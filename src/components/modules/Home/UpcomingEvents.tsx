    ...
    <section className="px-8 max-w-[1440px] mx-auto mb-40 overflow-hidden">
      <div className="flex justify-between items-end mb-12">
        ...
        <div className="flex gap-2">
          <button className="w-10 h-10 rounded-full border border-[#c0c9c3] flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all">
      ...
      <div className="flex gap-6 overflow-x-auto no-scrollbar pb-8">
        {isLoading ? (
          <div className="min-w-[320px] h-[350px] bg-white rounded-xl animate-pulse" />
        ) : events.length === 0 ? (
          <p className="text-[#404944]">No upcoming events at the moment.</p>
        ) : (
          events.map((event: IEvent) => (
            <div key={event.id} className="min-w-[320px] max-w-[320px] bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="aspect-video bg-[#edeeeb] overflow-hidden">
                <img
                  alt={event.title}
                  className="w-full h-full object-cover"
                  src={event.bannerImage || "https://lh3.googleusercontent.com/aida-public/AB6AXuBn-aY_OGBpsCgwIAvt5oHMNsKkTntzIqf88bYFE3IqA_raDyaq9Bj3W4sGkzRg8rGudmTyLWud7dveNrOzx_QSlLyAsQWAwpTEpReL1JnD2DPjBYtNW2W-4-TFzO9iLaPNiEKRq6F0lW_VVs-k9z8b4EsYSdC970s95VDEhaqvyp7KVn0BczNU_3skrgpYByaku6VH4fh_sEcnBuEOXaeomCy30dvXhVz641G4_8PfA-zUVIoczk-bgQ30FKU4UVl-fv_cNbtZsjAg"}
                />
              </div>
              <div className="p-6">

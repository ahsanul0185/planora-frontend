...
import { updateEventCategory } from "@/services/eventCategory.services";
...
const UpdateCategoryModal = ({
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: (formData: FormData) =>
      updateEventCategory(category!.id, formData),
    onSuccess: (res) => {
      if (res.success) {
  ...
  // Re-sync form when category changes
  useEffect(() => {
    if (category) {
    ...
    >
      <DialogContent className="max-w-md p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            Edit the event category details and icon.
          </DialogDescription>
        </DialogHeader>

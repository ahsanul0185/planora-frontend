...
import { loginZodSchema, registerZodSchema, verifyEmailZodSchema, changePasswordZodSchema, forgotPasswordZodSchema, resetPasswordZodSchema, ILoginPayload, IRegisterPayload, IVerifyEmailPayload, IChangePasswordPayload, IForgotPasswordPayload, IResetPasswordPayload } from "@/zod/auth.validation";
...
export async function forgotPassword(payload: IForgotPasswordPayload): Promise<any> {
    const parsedPayload = forgotPasswordZodSchema.safeParse(payload);
    if (!parsedPayload.success) {
        return { success: false, message: parsedPayload.error.issues[0].message || "Invalid email" };
    }
...
export async function resetPassword(payload: IResetPasswordPayload): Promise<any> {
